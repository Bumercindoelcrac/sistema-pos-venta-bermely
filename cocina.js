// cocina.js - Sistema específico para cocina
import { 
  db,
  collection,
  onSnapshot,
...
class SistemaCocina {
  ...
  inicializar() {
    this.escucharPedidosCocina();
    this.escucharPedidosListos();
  }
  
  // Escuchar pedidos para cocina
  ...
  escucharPedidosListos() {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      const container = document.getElementById('pedidosListosCocina');
      if (container) {
  ...
  renderizarPedidosPendientes() {
    const container = document.getElementById('pedidosPendientesCocina');
    if (!container) return;
    ...
    if (this.pedidosPendientes.length === 0) {
      container.innerHTML = `
        <div class="sin-pedidos">
          ...
          <p>Los pedidos aparecerán aquí cuando los meseros los envíen</p>
        </div>
      `;
      return;
    }
        ...
        <div class="pedido-card" data-pedido-id="${pedido.id}">
          <div class="pedido-header">
            ...
            <div class="pedido-acciones">
              <button class="btn btn-warning btn-iniciar" onclick="sistemaCocina.iniciarPreparacion('${pedido.id}')">
  ...
  renderizarPedidosEnPreparacion() {
    const container = document.getElementById('pedidosEnPreparacionCocina');
    if (!container) return;
      ...
      html += `
        <div class="pedido-card en-preparacion" data-pedido-id="${pedido.id}">
          <div class="pedido-header">
            ...
            <div class="pedido-acciones">
              <button class="btn btn-success btn-listo" onclick="sistemaCocina.marcarComoListo('${pedido.id}')">
...
const sistemaCocina = new SistemaCocina();

// Hacer disponible globalmente
window.sistemaCocina = sistemaCocina;

export { sistemaCocina };